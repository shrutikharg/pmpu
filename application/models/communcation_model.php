 ...
 * @author a
 */
class communcation_model extends CI_Model {
    ...
    public function get_message() {
        ...
        $this->db->order_by('m.created_by', 'desc');
        $query = $this->db->get();
        return $query->result();
    }

    public function get_subject_specific_message($master_id) {
        $this->db->select('m.message,u.email,concat_ws(u.first_name," ",u.last_name) as full_name,m.created_at',FALSE);
        $this->db->from('message m');
        $this->db->join('message_receiver mr', 'm.id=mr.message_id');
        $this->db->join('users u', 'u.id=m.created_by');
        $this->db->where('m.msg_master_id', $master_id);
        $this->db->where('mr.receiver_id', $this->session->userdata('company_id'));
        $query = $this->db->get();
        return$query->result();
    }
