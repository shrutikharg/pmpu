...
//index
    ...
    public function update() {
        ...
        if ($this->form_validation->run()) {
            ...
            $coursedescp = $data['courses'];
            //var_dump($coursedescp);
            //exit ;			
            ...
            if ($this->csvimport->get_array($file_path)) {
                ...
                foreach ($csv_array as $row) {
                    ...
                    if ($is_present) {
                        ...
                        $assignuser = $this->companycoursesassign_model->get_userdetailsinfouser($row['email']);
                        //var_dump($assignuser);
                        $insertassigned_course = array(
                            'courseid' => $id,
                            'assignuserid' => $assignuser[0]['id'],
                            'assignedby_user' => $userid,
                            'certificate_flag' => 'N',
                            'created_date' => date('Y-m-d'),
                            'created_by' => $this->session->userdata('user_name'),
                        );
                        //exit;
                        $this->companycoursesassign_model->insert_assignedcourse($insertassigned_course);
                        $message = 'Hello,';
                        $message .="<br/>";
                        $message .='Glad to have you on CoolAcharya. Follow this link http://coolacharya.com/companyadminapp/employee/login  to start your course assignment.';
                        $message .="<br/><br/>";
                        $message .='Course Name of - ' . $coursedescp[0]['course_name'];
                        $message .="<br/><br/>";
                        $message .='Thank You.';
                        ...
                        $empnewid = $this->db->insert_id();
                        $insert_datanew = array(
                            'user_id' => $empnewid,
                            'group_id' => '7',
                        );
                        $this->companycoursesassign_model->insert_usergroupcsv($insert_datanew);

                        $insert_assigned_course = array(
                            'courseid' => $id,
                            'assignuserid' => $empnewid,
                            'assignedby_user' => $userid,
                            'certificate_flag' => 'N',
                            'created_date' => date('Y-m-d'),
                            'created_by' => $this->session->userdata('user_name'),
                        );
                        $this->companycoursesassign_model->insert_assignedcourse($insert_assigned_course);

                        $message = 'Hello,';
                        $message .="<br/><br/>";
                        $message .='Glad to have you on CoolAcharya.';
                        $message .="<br/>";
                        $message .='Follow this link http://coolacharya.com/companyadminapp/employee/login  to start your course assignment.';
                        $message .="<br/><br/>";
                        $message .='Course Name of-' . $coursedescp[0]['course_name'];
                        $message .="<br/><br/>";
                        $message .='Username : ' . $row['email'];
                        $message .="<br/>";
                        $message .='Password : ' . $stringpass;
                        $message .="<br/><br/>";
