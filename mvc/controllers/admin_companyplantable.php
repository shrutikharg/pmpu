<?php
class Admin_companyplantable extends CI_Controller {
    ...
    * @constant string
    */
    const VIEW_FOLDER = 'admin_company/plantable';
    ...
    */
    public function index()
    {
        ...
        if($search_string !== false && $order !== false || $this->uri->segment(3) == true){ 
            ...
            $data['count_products']= $this->companycmspage_model->count_cmspage($userid,$search_string, $order,$userid);
            $config['total_rows'] = $data['count_products'];

            //fetch sql data into arrays
            if($search_string){
                if($order){
                    $data['cmspage'] = $this->companycmspage_model->get_cmspage($userid,$search_string, $order, $order_type, $config['per_page'],$limit_end);        
                }else{
                    $data['cmspage'] = $this->companycmspage_model->get_cmspage($userid,$search_string, '', $order_type, $config['per_page'],$limit_end);           
                }
            }else{
                if($order){
                    $data['cmspage'] = $this->companycmspage_model->get_cmspage($userid,'', $order, $order_type, $config['per_page'],$limit_end);        
                }else{
            ...
            //fetch sql data into arrays
            $data['count_products']= $this->companycmspage_model->count_cmspage($userid);
            $data['cmspage'] = $this->companycmspage_model->get_cmspage($userid,'', '', $order_type, $config['per_page'],$limit_end);        
            $config['total_rows'] = $data['count_products'];
	...
	$data['footerdata']= $this->companycmspage_model->list_cmspage($userid,$usernm);
        ...
        //load the view
        $data['main_content'] = 'admin_company/plantable/listplan';
    ...
    public function add()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST')
        {
            ...
            if ($this->form_validation->run())
            {
                ...
                if($this->companycmspage_model->store_cmspage($data_to_store)){
    ...
    */
    public function update()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST')
        {
            ...
            if ($this->form_validation->run())
            {
               ...
               $data_to_store = array(
                ...
                //if the insert has returned true then we show the flash message
                if($this->companycmspage_model->update_cmspage($id, $data_to_store) == TRUE){
        ...
        //product data 
        $data['cmspage'] = $this->companycmspage_model->get_cmspage_by_id($id);
		$data['footerdata']= $this->companycmspage_model->list_cmspage($userid,$usernm);
