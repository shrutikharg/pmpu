...
/* ---------- Pie Charts ---------- */
    ...
    public function column_chart_basic()
    {
		...
		$data['courses'] = $this->companycourses_model->get_graphcourses($userid);
		...
		{
        $this->gcharts->DataTable('Inventory')->addColumn('number',$data['courses'][$i]['clickcourse_date'], $data['courses'][$i]['course_name']);
		
        $this->gcharts->DataTable('Inventory')->addRow(array(
                          $data['courses'][$i]['course_name'],                          
							($data['courses'][$i]['cnt']),						 
        ...
        $this->load->view('gcharts/column_chart_basic',$data);
    }
